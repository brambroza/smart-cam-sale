...
describe('PurchasesService.record', () => {
  ...
  it('rejects an empty cart', async () => {
    const svc = new PurchasesService(prismaMock());
    await expect(svc.record({ memberId: 'm1', items: [] })).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });
  ...
  it('rejects qty outside 1-99 and non-integers', async () => {
    ...
    for (const qty of [0, 100, 1.5, -1]) {
      await expect(
        svc.record({ memberId: 'm1', items: [{ productId: 'p1', qty }] }),
  ...
  it('rejects an unknown product', async () => {
    const svc = new PurchasesService(prismaMock());
    await expect(
      svc.record({ memberId: 'm1', items: [{ productId: 'nope', qty: 1 }] }),
    ).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('rejects an unknown member', async () => {
    ...
    await expect(
      svc.record({ memberId: 'ghost', items: [{ productId: 'p1', qty: 1 }] }),
