    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          >
            <div className="p-5 border-b border-white/10 sticky top-0 bg-ink-900/85 backdrop-blur-xl z-10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <PackageOpen className="w-5 h-5 text-neon-cyan" />
                  <h2 className="font-display font-bold text-lg text-slate-100">แค็ตตาล็อกสินค้า</h2>
                  <span className="text-xs text-slate-500 font-mono">({products.length})</span>
                </div>
                <button onClick={onClose} className="p-2 rounded-lg hover:bg-white/10 transition text-slate-300">
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              {categories.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-1.5">
                  <CatChip active={category === null} onClick={() => setCategory(null)}>ทั้งหมด</CatChip>
                  {categories.map((c) => (
