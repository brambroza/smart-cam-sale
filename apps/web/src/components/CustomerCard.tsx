...
export function CustomerCard({
  ...
  return (
    <motion.div
      ...
      className={cn('glass-strong shadow-card relative overflow-hidden', compact ? 'p-3' : 'p-5')}
    >
      <div className="absolute -top-24 -right-24 w-64 h-64 rounded-full bg-gradient-to-br from-violet-500/30 to-cyan-400/20 blur-3xl pointer-events-none" />
      ...
      <div className="relative flex items-start gap-4">
        <motion.div
          ...
          animate={{ scale: 1 }}
          className={cn(
            'w-16 h-16 rounded-2xl grid place-items-center shadow-glow flex-shrink-0',
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h2 className="font-display text-2xl font-bold truncate text-slate-100">
...
function HoldChip({ since }: { since: number }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-violet-500/15 border border-violet-400/30 text-[10px] text-violet-200">
